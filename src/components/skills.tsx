      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
          ...
          {columns.map(({ key, data, accent }) => {
            const dotCls =
              accent === "lens"
                ...
                : "bg-bone-100";
            return (
              <Reveal key={key} delay={0.08}>
                <div className="flex h-full flex-col rounded-2xl border border-line bg-ink-850 p-6 sm:p-7">
                  ...
                  <ul className="mt-6 space-y-2.5">
                    {data.items.map((item) => (
                      <li
                        key={item}
                        className="flex items-center justify-between gap-3 border-b border-line pb-2.5 text-sm text-fog-300"
                      >
                        <span>{item}</span>
                      </li>
                    ))}
        ...
        <RevealStagger className="mt-12" stagger={0.05}>
          ...
          <div className="flex flex-wrap gap-2.5">
            {toolkit.map((tool) => (
              <RevealItem key={tool}>
                <span className="inline-flex items-center gap-2 rounded-full border border-line-strong bg-wash px-4 py-2 font-mono text-xs text-fog-400 transition-colors duration-300 hover:border-lens-400/40 hover:text-lens-300">
                  <span aria-hidden className="flex h-4 w-4 items-center justify-center rounded-[5px] bg-wash font-sans text-[9px] font-bold text-bone-100/70">
